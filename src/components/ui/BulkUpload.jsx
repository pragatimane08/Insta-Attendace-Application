    ...
    for (let i = 0; i < data.length; i++) {
      for (const field of requiredFields) {
        ...
        if (!value || value.toString().trim() === '') {
          return {
            isValid: false,
            error: `Row ${i + 2} is missing or has empty value for field: "${field}".`,
          };
        }
        ...
        if (field === 'Phone') {
          ...
          if (!/^\d{10}$/.test(phoneStr)) {
            return {
              isValid: false,
              error: `Row ${i + 2}: Invalid phone number "${phoneStr}". It should be exactly 10 digits.`,
            };
          }
        }
        ...
        if (field === 'Email') {
          ...
          if (!/^[^\s@]+@gmail\.com$/.test(email)) {
            return {
              isValid: false,
              error: `Row ${i + 2}: Invalid email "${email}". Email should be a valid Gmail address.`,
            };
          }
        }
      }
